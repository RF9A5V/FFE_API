  ...
  Item.findOne({ '_id': req.params.id }).populate("interested_count").exec(function(err, item){
    ...
    for(var i = 0; i < item.interested_count.length; i ++){
      if(item.interested_count[i]._id == req.session.uid){
        ...
        item.save(function(err){
          removed = true;
          res.json({status: "success", message: "Didn't break", item: { count: item.interested_count.length, user_likes: !removed}})
          res.end();
        })
      }
    }
    if(!removed){
      User.findOne({ "_id": req.session.uid }, function(err, user){
        ...
        item.save(function(err) {
          res.json({status: "success", message: "Didn't break", item: { count: item.interested_count.length, likes: !removed}})
          res.end();
        })
      })
    }
